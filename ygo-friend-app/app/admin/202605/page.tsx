...
import type { Match, Season } from '@/app/tournament/types';
...
const TOURNAMENT = TOURNAMENTS.find(t => t.id === '202605')!;
...
function DateInput({ value, label, onChange }: { value: string; label: string; onChange: (v: string) => void }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', border: '1px solid #d1d5db', borderRadius: '8px', background: '#fff', overflow: 'hidden' }}>
        <input
          ref={ref}
          type="date"
          value={value}
...
function HoverButton({
  ...
  const variants: Record<string, React.CSSProperties> = {
    ...
    'outline-danger': { background: hover && !disabled ? '#fee2e2' : 'transparent', color: '#dc2626', border: '1px solid #fca5a5', boxShadow: 'none' },
...
// ── Main ─────────────────────────────────────────────────────────────────────
export default function Admin202605Page() {
  ...
  useEffect(() => {
    ...
    fetch('/api/tournaments/202605/meta')
      .then(r => r.json())
  ...
  const handleInfoSave = async () => {
    ...
    try {
      ...
      const res = await adminFetch('/api/tournament', { method: 'PATCH', body: JSON.stringify({ name: editSeasonName.trim(), players: updatedPlayers }) });
      if (res.status === 401) { handleUnauthorized(); return; }
  ...
  if (mode === 'generate' || !season) {
    return (
      <main style={{ maxWidth: '560px', margin: '0 auto', padding: '2rem 1.5rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', margin: '20px 0 24px' }}>
          <h1 style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#1e293b' }}>スケジュール生成</h1>
          {season && (
            <HoverButton onClick={() => setMode('view')} variant="ghost">← キャンセル</HoverButton>
          )}
        </div>
        {genError && (
        ...
        <Card style={{ marginBottom: '24px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {dates.map((date, i) => (
              <DateInput key={i} label={`第${i + 1}回`} value={date} onChange={v => setDates(prev => prev.map((d, j) => j === i ? v : d))} />
            ))}
          </div>
        </Card>
        <HoverButton onClick={handleGenerate} disabled={generating} variant="secondary" style={{ width: '100%', padding: '12px', fontSize: '1rem' }}>
    ...
    <main style={{ maxWidth: '980px', margin: '0 auto', padding: '2rem 1.5rem' }}>
      ...
      <Card style={{ marginTop: '20px', marginBottom: '16px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '10px 16px', alignItems: 'center', marginBottom: '16px' }}>
          ...
          <span style={{ fontSize: '0.95rem', color: '#1e293b' }}>個人戦</span>
          ...
          <span style={{ fontSize: '0.82rem', color: '#64748b', fontWeight: 600 }}>優勝者</span>
          <input
            value={metaWinner}
            onChange={e => setMetaWinner(e.target.value)}
            disabled={metaStatus !== 'finished'}
            placeholder={metaStatus !== 'finished' ? 'ステータスを「終了」にすると入力できます' : '優勝者名を入力'}
            style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <HoverButton onClick={handleMetaSave} disabled={metaSaving} variant="primary" style={{ padding: '8px 20px' }}>
      ...
      {/* 大会データ設定 — 終了時はロック */}
      <div style={{ opacity: isFinished ? 0.5 : 1, pointerEvents: isFinished ? 'none' : 'auto' }}>
        {/* Header card */}
        <Card style={{ marginBottom: '24px' }}>
          {editingInfo ? (
            <div style={{ marginBottom: '20px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
                <span style={{ fontSize: '0.8125rem', fontWeight: 700, color: '#374151', textTransform: 'uppercase', letterSpacing: '0.04em' }}>✏️ 基本情報を編集</span>
                <HoverButton onClick={() => setEditingInfo(false)} variant="ghost" style={{ padding: '5px 12px', fontSize: '0.8125rem' }}>キャンセル</HoverButton>
              </div>
              <div style={{ marginBottom: '14px' }}>
                <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 600, color: '#64748b', marginBottom: '6px' }}>シーズン名</label>
                <input
              ...
              <div style={{ marginBottom: '16px' }}>
                <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 600, color: '#64748b', marginBottom: '8px' }}>プレイヤー名</label>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '10px' }}>
                {infoToast !== 'idle' && (
                  <span style={{
                    display: 'inline-flex', alignItems: 'center', gap: '5px', padding: '4px 12px', borderRadius: '20px', fontSize: '0.8125rem', fontWeight: 600,
                    ...(infoToast === 'saving' ? { background: '#eff6ff', color: '#2563eb', border: '1px solid #bfdbfe' }
                      : infoToast === 'saved' ? { background: '#f0fdf4', color: '#16a34a', border: '1px solid #bbf7d0' }
                ...
                <HoverButton onClick={handleInfoSave} disabled={infoToast === 'saving' || !editSeasonName.trim() || editPlayerNames.some(n => !n.trim())} variant="primary" style={{ padding: '8px 24px' }}>
                  保存
                </HoverButton>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap', marginBottom: '20px' }}>
              <div>
                <h1 className="reisho" style={{ fontSize: '1.75rem', fontWeight: 'bold', color: '#1e293b' }}>{season.name}</h1>
                <p style={{ color: '#64748b', fontSize: '0.875rem', marginTop: '4px' }}>
                  参加プレイヤー: {season.players.map(p => p.name).join('・')}
                </p>
              </div>
              <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexShrink: 0 }}>
                <HoverButton onClick={handleInfoEdit} variant="ghost">✏️ 編集</HoverButton>
                <HoverButton onClick={() => setMode('generate')} variant="outline-danger">🔄 再生成</HoverButton>
                <HoverButton onClick={() => setShowDeleteConfirm(true)} variant="danger">🗑️ 削除</HoverButton>
              </div>
            </div>
          )}
          ...
          <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: '18px' }}>
            ...
            <textarea
              ...
              rows={5}
              style={{ width: '100%', boxSizing: 'border-box', border: '1px solid #d1d5db', borderRadius: '8px', padding: '10px 14px', fontSize: '0.9375rem', lineHeight: '1.7', color: '#1e293b', background: '#f8fafc', resize: 'vertical', outline: 'none', fontFamily: '"Noto Sans JP", "Hiragino Sans", "Yu Gothic", sans-serif' }}
            />
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '10px' }}>
              <span style={{ fontSize: '0.75rem', color: description.length >= 900 ? '#d97706' : '#94a3b8' }}>{description.length} / 1000 文字</span>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                {descToast !== 'idle' && (
                  <span style={{
                    display: 'inline-flex', alignItems: 'center', gap: '5px', padding: '4px 12px', borderRadius: '20px', fontSize: '0.8125rem', fontWeight: 600,
                    ...(descToast === 'saving' ? { background: '#eff6ff', color: '#2563eb', border: '1px solid #bfdbfe' }
                      : descToast === 'saved' ? { background: '#f0fdf4', color: '#16a34a', border: '1px solid #bbf7d0' }
                      : { background: '#fff1f2', color: '#dc2626', border: '1px solid #fecdd3' }),
                  }}>
                ...
                <HoverButton onClick={handleDescriptionSave} disabled={descToast === 'saving'} variant="primary" style={{ padding: '7px 20px' }}>保存</HoverButton>
              </div>
            </div>
          </div>
        </Card>
        ...
        <Card>
          <SessionTabs season={season} isAdmin onSessionSave={handleSessionSave} onDateChange={handleDateChange} />
        </Card>
      </div>

      {/* 削除確認モーダル */}
      {showDeleteConfirm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 500, padding: '1rem' }}>
          <Card style={{ maxWidth: '400px', width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            <h2 style={{ fontSize: '1.125rem', fontWeight: 700, color: '#1e293b', marginBottom: '12px' }}>🗑️ スケジュールを削除</h2>
            <p style={{ color: '#475569', fontSize: '0.9375rem', marginBottom: '8px' }}>
              <strong>{season.name}</strong> のすべてのデータを削除します。
            </p>
            <p style={{ color: '#ef4444', fontSize: '0.875rem', marginBottom: '24px' }}>この操作は取り消せません。</p>
            <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
              <HoverButton onClick={() => setShowDeleteConfirm(false)} variant="ghost">キャンセル</HoverButton>
              <HoverButton onClick={handleDelete} disabled={deleting} variant="danger">{deleting ? '削除中...' : '削除する'}</HoverButton>
            </div>
          </Card>
        </div>
      )}
